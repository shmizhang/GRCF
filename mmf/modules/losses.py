...
@registry.register_loss("conf1_loss")
class SoftmaxAnchorPredLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        ...
        gt_anchor = sample_list['ocr_confidence']
        #anchor_pred
        anchor_pred = model_output["conf1_pred"]  # [B, M]
        loss_mask = model_output["ocr_mask"]        # [B, M]
        anchor_loss = - (torch.squeeze(gt_anchor) * F.log_softmax(anchor_pred, dim=-1))
...
@registry.register_loss("conf2_loss")
class GraphPredLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
      ...
      # losses = F.binary_cross_entropy_with_logits(
        ...
        #graph_pred
        scores = model_output["conf2_pred"]  
        #scores=model_output["mmt_ocr_confidence_score"]  
        targets = torch.squeeze(sample_list['ocr_confidence']) > 0
...
@registry.register_loss("mmt_ocr_loss")
class MmtPredLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.one = torch.Tensor([1.])
    ...
    def forward(self, sample_list, model_output):
      ...
      # losses = F.binary_cross_entropy_with_logits(
        #     scores, targets.float(), reduction="none"
        # )
        #scores = model_output["graph_pred"]  
        loss_mask = model_output["ocr_mask"]
        #print(model_output.keys())
        scores = model_output["mmt_ocr_score"]
        targets = torch.squeeze(sample_list['ocr_confidence']) > 0
        losses = F.binary_cross_entropy(
            scores, targets.float(), reduction="none"
        )
...
@registry.register_loss("gcn_ocr_cross_entropy")
class Gcnocr_CrossEntropyLoss(nn.Module):
    ...
    def forward(self, sample_list, model_output):
        #print(model_output["ocr_emb"].size())
        #print(model_output["relation_ocr"].size())
        tmp=F.softmax(torch.matmul(model_output["ocr_emb"],model_output["ocr_emb"].permute(0,2,1)),dim=2)
        tmp=tmp.reshape(tmp.size(0),-1)
        target= model_output["relation_ocr"].reshape(tmp.size(0),-1)
        #print("ocr size")
        #print(tmp)
        #print(target.size())
        losses = self.loss_fn(tmp,target)
        return losses

@registry.register_loss("gcn_obj_cross_entropy")
class Gcnobj_CrossEntropyLoss(nn.Module):
    def __init__(self):
        super().__init__()
        self.loss_fn = nn.MSELoss()
        #self.loss_fn=nn.KLDivLoss(reduction="batchmean")
    def forward(self, sample_list, model_output):
        tmp=F.softmax(torch.matmul(model_output["obj_emb"],model_output["obj_emb"].permute(0,2,1)),dim=2)
        tmp=tmp.reshape(tmp.size(0),-1)
        target= model_output["relation_obj"].reshape(tmp.size(0),-1)
        #print("obj size")
        #print(tmp)
        #print(target.size())
        losses = self.loss_fn(tmp,target)
